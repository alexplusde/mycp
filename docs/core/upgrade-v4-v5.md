...
Quelle: https://redaxo.org/doku/main/upgrade-v4-v5
...
- Den Adminer in REDAXO 4 in neuem Tab aufrufen.

- Im Adminer von REDAXO 4 oben links auf `Exportieren` klicken.

- Alle Tabellen und Daten wegklicken (im Tabellenkopf).

- Nur die `Daten` auswählen deren Tabelle mit `yconverter_` beginnen. ***Es darf keine Checkbox bei Tabelle aktiviert werden***.

- Button `Exportieren` klicken.

- Erstellte Daten in die Zwischenablage kopieren

- Im Adminer von REDAXO 5 oben links SQL-Kommando klicken und das Kopierte in das Textfeld einfügen.

- Nach `yconverter_` im Textfeld suchen und löschen. (ggf. in einem Texteditor per suchen und Ersetzen)

- Den Button Ausführen klicken.
...
`$REX['REDAXO']`
`rex::isBackend()` zusätzlich `rex::isFrontend()` um zu prüfen ob Frontend
...
`$REX['PERM'][] = 'myperm[]'`
`rex_perm::register('myperm[]', $name = null)` 
...
`$REX['HTDOCS_PATH']` 
 ...
 `$REX['MEDIAFOLDER']` 
 `$REX['FRONTEND_FILE']`
`rex_path` [siehe hier](/doku/main/pfade)
...
`$sql->setWhere('myid = :id OR abc = :abc', array(':id' => 3, ':abc' => 'zdf'))` 
oder
...
`$form = new my_form_class('table', 'legend', 'id="'.$id.'"', 'post', false);` 
oder
...
Des Weiteren können Daten im Ordner `redaxo/data/addons/$addonName` abgelegt werden, der Pfad ist über `rex_path::addonData($addon)` und `rex_addon::get($addon)->getDataPath()` , bzw. `rex_addon::get('addonkey')->getDataPath()` erreichbar.
                [**Artikel bearbeiten](https://github.com/redaxo/docs/edit/main/upgrade-v4-v5.md)
