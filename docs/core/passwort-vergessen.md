...
Quelle: https://redaxo.org/doku/main/passwort-vergessen
...
- In der Datei `/redaxo/data/core/config.yml` muss `setup: false` auf `setup: true`gesetzt werden. 

- Danach die Website unbedingt sofort aufrufen! (Der Setup-Vorgang ist für alle Besucher sichtbar, daher ist Eile geboten!)

- Durchgehen des Setups, Schritt 4 so belassen wie es ist.

- In Schritt 5 (Datenbank) `Datenbank existiert schon` auswählen.

- Im nächsten Schritt einen neuen Nutzer anlegen.

- Abschließen und dann in REDAXO einloggen. 

## Änderung eines Passworts über die Konsole

Ein Passwort kann sehr einfach über die [Konsole](/doku/main/console) aktualisiert werden. Hierzu in der Konsole ins Verzeichnis `/redaxo/bin` der REDAXO-Installation wechseln und diesen Befehl eingeben, um das Passwort zurückzusetzen.
...
Wurde das neue Passwort gespeichert, sollte folgende Meldung erscheinen: `[OK] Saved new password for user XYZ`
                [**Artikel bearbeiten](https://github.com/redaxo/docs/edit/main/passwort-vergessen.md)
